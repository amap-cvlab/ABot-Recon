...
class ABotReconNetwork(Pi3):
    ...
    def __init__(self, **kwargs):
        ...
        self.use_chunk_flex_attention = bool(
            kwargs.pop("use_chunk_flex_attention", False)
        )
        self.use_chunk_flex_compile = bool(
            kwargs.pop("use_chunk_flex_compile", False)
        )
        ...
        for i in range(1, len(self.decoder), 2):
            ...
            blk.attn.use_chunk_flex_attention = self.use_chunk_flex_attention
            blk.attn.use_chunk_flex_compile = self.use_chunk_flex_compile
                    ...
                    def _global_attn_cp(
                        ...
                        *,
                        ...
                        bias_sig = (
                            ...
                            bool(getattr(_attn, "use_chunk_flex_attention", False)),
                        )
                        use_flex = bool(
                            getattr(_attn, "use_chunk_flex_attention", False)
                        ) and h.is_cuda
                        if use_flex:
                            # FlexAttention represents the temporal window as a
                            # BlockMask; never materialize the quadratic SDPA bias.
                            _bias_cache.pop("bias", None)
                            _bias_cache["bias_sig"] = bias_sig
                            bias_cached = None
                        elif _bias_cache.get("bias_sig") != bias_sig or "bias" not in _bias_cache:
    ...
    def _load_deferred_streaming_checkpoint(self, ckpt: str) -> None:
        ...
        ckpt_path = str(ckpt)
        print(
            "[ABotReconNetwork] Reading deferred checkpoint (local path; can be slow)…\n"
            f"      path={ckpt_path}",
            flush=True,
        )
