...
class BlockRope(nn.Module):
    ...
    def forward(
        ...
        def attn_residual_func(x_in: Tensor):
            if use_cache:
        ...
        if use_cache:
            attn_out, new_kv = attn_residual_func(x)
            x = x + attn_out
            x = x + ffn_residual_func(x)
            return x, new_kv
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            if attn_mask is not None:
                raise NotImplementedError("stochastic depth with causal attn_mask is not supported")
            x = drop_add_residual_stochastic_depth(
                x,
                residual_func=lambda t: attn_residual_func(t),
                sample_drop_ratio=self.sample_drop_ratio,
            )
            x = drop_add_residual_stochastic_depth(
                x,
