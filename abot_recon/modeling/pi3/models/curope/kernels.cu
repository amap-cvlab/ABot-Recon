...
__device__ __forceinline__ float quantize_like_reference(const float value)
{
        ...
        // const int N, const int H, const int D )
{
    ...
    if (threadIdx.x < Q)
        shared_inv_freq[threadIdx.x] = quantize_like_reference<scalar_t>(
            fwd / powf(base, threadIdx.x/float(Q)));
    __syncthreads();
    ...
    const float freq = quantize_like_reference<scalar_t>(
        pos[blockIdx.x*2+X] * shared_inv_freq[threadIdx.x % Q]);
    const float cos = quantize_like_reference<scalar_t>(cosf(freq));
    const float sin = quantize_like_reference<scalar_t>(sinf(freq));
    /*
    ...
    */
    ...
    for (int h = 0; h < H; h++)
    {
        ...
        // Match the reference dtype's rounding after each multiply and add.
        if ((threadIdx.x % (D/2)) < Q) {
            const float uc = quantize_like_reference<scalar_t>(u*cos);
            const float neg_vs = quantize_like_reference<scalar_t>((-v)*sin);
            tokens[b][n][h][threadIdx.x] = static_cast<scalar_t>(
                quantize_like_reference<scalar_t>(uc + neg_vs));
        } else {
            const float vc = quantize_like_reference<scalar_t>(v*cos);
            const float us = quantize_like_reference<scalar_t>(u*sin);
            tokens[b][n][h][threadIdx.x] = static_cast<scalar_t>(
                quantize_like_reference<scalar_t>(vc + us));
        }
        __syncthreads();
    }
}
