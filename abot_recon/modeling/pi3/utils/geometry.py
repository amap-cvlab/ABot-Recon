...
def parse_depth_quantile_value(q) -> Optional[float]:
    """
    Normalize depth tail-clip strength for LOGER-style supervision (positive depths only).

    - ``(0, 1)``: ``np.quantile`` fraction, e.g. ``0.98`` == 98th percentile (same as
      ``np.percentile(..., 98)`` in CUT3R loaders like DL3DV / TartanAir).
    - ``(1, 100]``: **percentile index** as in LOGER wording (e.g. ``90`` or ``98``).
    - ``>= 1`` after scaling, or legacy ``1.0``: **no clip** (CUT3R ``quantile=1.0``).
    """
    if q is None:
        return None
...
def apply_depth_upper_quantile_cap(
    depthmap: np.ndarray,
    ...
    *,
    ...
    """
    LOGER / CUT3R dataset-style depth filtering: mask depths above an empirical upper tail.

    Among pixels with depth > 0, ``thr = np.quantile(depth, q)`` (``q`` in (0,1), e.g. 0.98).
    Pixels with depth > thr are set to 0. Pair with a fixed ``z_far`` cap in
    ``depthmap_to_absolute_camera_coordinates`` when ``z_far > 0`` (metric max depth); when
    ``z_far == 0``, use this instead of a fixed meter cap.

    - ``quantile is None`` or ``quantile >= 1.0``: no change (float32 view, copy only if needed).
    - If fewer than ``min_valid`` positive-depth pixels, no change (stable statistics).
    """
    if quantile is None or quantile >= 1.0:
        return np.asarray(depthmap, dtype=np.float32)
    depthmap = np.asarray(depthmap, dtype=np.float32).copy()
    valid = depthmap > 0
    n = int(np.count_nonzero(valid))
    if n < int(min_valid):
        return depthmap
    thr = float(np.quantile(depthmap[valid], quantile))
    if not np.isfinite(thr) or thr <= 0:
        return depthmap
    outlier = valid & (depthmap > thr)
    depthmap[outlier] = 0.0
    return depthmap
