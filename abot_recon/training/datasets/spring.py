...
"""Spring sequences in the unmodified CUT3R processed layout."""
...
class Spring(MultiViewDataset):
    """Read sequence/{rgb/*.png,depth/*.npy,cam/*.npz} with metric depth.

    NPZ files contain ``intrinsics`` and camera-to-world ``pose``. The caller
    chooses the root; ``split`` does not partition its sequences. Interval
    parameters correspond to the development loader's min_stride/max_stride.
    """
    ...
    def __init__(
        ...
        for sequence in sorted(path for path in self.root.iterdir() if path.is_dir()):
            rgb = sequence / "rgb"
            if rgb.is_dir():
                names = sorted(path.stem for path in rgb.glob("*.png") if path.is_file())
    ...
    def _get_views(self, idx, resolution, rng, num_views, preserve_fov, sequence_aug):
        ...
        for position in positions:
            name = names[position]
            image = image_open(sequence / "rgb" / f"{name}.png")
            ...
            image, depth, intrinsics = self._crop_resize_if_necessary(
                ...
                info=str(sequence / "rgb" / f"{name}.png"),
            )
            views.append(dict(
